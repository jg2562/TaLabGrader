...
function_str_name = "Update and Get email"
...
def check_function_exists(studentCode):
    try:
        studentCode.update_contact_email()
...
def check_email_function(studentCode):
    try:
        people_amount = 100
        ...
        for person in fake_people:
            new_num = generate_email()
            studentCode.update_contact_email(contacts, person[0], person[1], new_num)
            if studentCode.get_contact_email(contacts, person[0], person[1]) != new_num:
...
def get_test_functions():
    return [(check_function_exists, 0.25, "Checking {} function exists".format(function_str_name)),
            (check_email_function, 0.75, "Veriying {} functions".format(function_str_name))]
