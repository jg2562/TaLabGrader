...
function_str_name = "Update and Get phone"
...
def check_function_exists(studentCode):
    try:
        studentCode.update_contact_number()
    ...
    try:
        studentCode.get_contact_number()
    except AttributeError:
...
def check_phone_function(studentCode):
    try:
        people_amount = 100
        ...
        for person in fake_people:
            new_num = generate_number()
            studentCode.update_contact_number(contacts, person[0], person[1], new_num)
            if studentCode.get_contact_number(contacts, person[0], person[1]) != new_num:
...
def get_test_functions():
    return [(check_function_exists, 0.25, "Checking {} function exists".format(function_str_name)),
            (check_phone_function, 0.75, "Veriying {} functions".format(function_str_name))]
